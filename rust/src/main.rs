...
use bitcoin_wallet::account::{
    Account, AccountAddressType, MasterAccount, MasterKeyEntropy, Unlocker, Seed,
};
use bitcoin_wallet::mnemonic::Mnemonic;
use bitcoin_wallet::context::SecpContext;
use bitcoin_wallet::*;
use bitcoin::util::{bip32::{ExtendedPubKey, ChildNumber}, base58, address::Address as Addr};
use hex;

lazy_static! {
    // coin types include 2 to allow bad test case
    static ref BIP44Path: Regex = Regex::new(r"^m/(44|49|84)'/([012])'/(\d+)'/([01])/(\d+)$").unwrap();
}
...
async fn main() -> tide::Result<()> {
    ...
    app.at("/api/multisig-p2sh-address").post(handle_segwit_addr);
...
struct ResponseHDSegwitAddr {
    address_root: AddressRoot,
    address: Address,
}
...
async fn handle_segwit_addr(mut req: Request<()>) -> tide::Result {
    ...
    if let Some(indexes) = BIP44Path.captures(&path) {
        ...
        let network = match coin_type {
            0 => Network::Bitcoin,
            1 => Network::Testnet,
            // 2 is allowed in BIP44Path regexp
            default => {
        ...
        let mut master = MasterAccount::from_seed(&seed, 0, network, &passphrase).unwrap();
        ...
        if let Ok(purpose) = u32::from_str(&indexes[1]) {
            match purpose {
                44u32 => {
                    ...
                    let ext_priv_key = unlocker.context().private_child(&by_change, ChildNumber::Normal {index}).unwrap();
                    let ext_pub_key: ExtendedPubKey = unlocker.context().extended_public_from_private(&ext_priv_key);
